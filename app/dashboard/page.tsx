...
function StatCard({ label, value, total, accent, icon, href }: {
  ...
  return (
    <Link href={href} className="group">
      <div className="relative overflow-hidden bg-white border border-slate-200 p-5 rounded-md shadow-xs group-hover:border-slate-300 group-hover:shadow-sm transition-all duration-200 h-full">
        <div className="flex items-start justify-between mb-4">
          <div className={`w-9 h-9 rounded-md flex items-center justify-center ${accent}`}>{icon}</div>
          {pct !== null && (
            <span className="font-mono text-[10px] font-bold text-slate-400 tracking-wider">{pct}%</span>
          )}
        </div>
        <p className="font-mono text-4xl font-bold text-slate-900 mb-1 tabular-nums">{value}</p>
        <p className="text-slate-500 text-sm font-medium">{label}</p>
        {pct !== null && (
          <div className="mt-3 h-1 bg-slate-100 overflow-hidden">
            <div className="h-full bg-navy-700 transition-all duration-700" style={{ width: `${pct}%` }} />
          </div>
        )}
      </div>
    </Link>
...
function QuickAction({ href, icon, label, desc, hoverColor }: {
  ...
  return (
    <Link href={href}>
      <div className={`bg-white border-2 border-slate-100 rounded-md p-4 flex items-center gap-3.5 transition-all duration-150 ${hoverColor}`}>
        <div className="w-9 h-9 bg-slate-100 rounded-md flex items-center justify-center text-slate-600 shrink-0">{icon}</div>
    ...
    return (
      <DashboardLayout>
        <div className="p-6 md:p-8 max-w-3xl">
          ...
          {/* Today's progress */}
          <div className="relative bg-navy-900 p-6 text-white mb-6 shadow-lg">
            <span className="absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 border-accent-400" />
            <span className="absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 border-accent-400" />
            <p className="text-white/60 text-xs font-semibold uppercase tracking-widest mb-3">{t("dashboard.todayPickups")}</p>
            {loading ? (
              <div className="h-10 bg-white/20 animate-pulse w-24" />
            ) : (
              <>
                <div className="flex items-end gap-2 mb-4">
                  <span className="font-mono text-5xl font-bold tabular-nums">{done}</span>
                  <span className="text-white/50 text-2xl font-bold mb-1">/ {total}</span>
                </div>
                <div className="h-1.5 bg-white/20 overflow-hidden mb-3">
                  <div className="h-full bg-accent-400 transition-all duration-700"
          ...
          {/* Next stop */}
          {!loading && next && (
            <div className="bg-white border border-navy-100 rounded-md p-5 mb-6 shadow-sm">
              <p className="text-xs font-bold text-navy-600 uppercase tracking-widest mb-3">{t("dashboard.nextStop")}</p>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-mono font-bold text-navy-700 text-sm mb-1">{next.trackingCode}</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Link href="/dashboard/mapa" className="group">
              <div className="bg-white border-2 border-slate-100 hover:border-navy-200 rounded-md p-5 transition-all duration-200 text-center">
                <div className="w-10 h-10 bg-navy-50 rounded-md flex items-center justify-center text-navy-600 mx-auto mb-3"><IconMap /></div>
            ...
            <Link href="/dashboard/mis-recogidas" className="group">
              <div className="bg-white border-2 border-slate-100 hover:border-indigo-200 rounded-md p-5 transition-all duration-200 text-center">
                <div className="w-10 h-10 bg-indigo-100 rounded-md flex items-center justify-center text-indigo-600 mx-auto mb-3"><IconList /></div>
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Link
              href="/recoger"
              className="inline-flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-4 py-2 rounded-md text-sm transition-colors shadow-sm"
            >
        ...
        {!loading && stats.pendingOld && stats.pendingOld > 0 ? (
          <div className="mb-5 bg-amber-50 border border-amber-200 rounded-md px-4 py-3 flex items-center gap-3">
            <div className="w-8 h-8 bg-amber-100 rounded-md flex items-center justify-center text-amber-600 shrink-0">
              <IconAlert />
            </div>
        ...
        {/* Today's progress */}
        {!loading && (
          <div className="bg-navy-50 border border-navy-100 rounded-md p-5 mb-7 flex items-center gap-5 flex-wrap">
            <div className="w-9 h-9 bg-navy-100 rounded-md flex items-center justify-center text-navy-700 shrink-0">
            ...
            {stats.todayTotal > 0 && (
              <div className="flex items-center gap-3 min-w-48 flex-1">
                <div className="flex-1 h-2 bg-white border border-navy-100 overflow-hidden shadow-inner">
                  <div className="h-full bg-navy-700 transition-all duration-700"
                    style={{ width: `${todayPct}%` }} />
                </div>
                <span className="font-mono text-sm font-bold text-navy-700 shrink-0 tabular-nums">{todayPct}%</span>
              </div>
            )}
          </div>
        )}
            ...
            <div className="bg-white rounded-md border border-slate-200 overflow-hidden shadow-xs">
              {loading ? (
                <div className="divide-y divide-slate-100">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div key={i} className="px-4 py-3.5 flex gap-3 items-center animate-pulse">
                      <div className="w-9 h-9 bg-slate-200 rounded-md shrink-0" />
                ...
                <div className="text-center py-10 text-slate-400">
                  <div className="w-10 h-10 bg-slate-100 rounded-md flex items-center justify-center mx-auto mb-3"><IconBox /></div>
                ...
                <div className="divide-y divide-slate-100">
                  {recent.map((p) => (
                    <Link key={p.id} href={`/dashboard/solicitudes/${p.id}`}
                      ...
                      <div className="w-9 h-9 bg-slate-100 group-hover:bg-indigo-100 rounded-md flex items-center justify-center shrink-0 text-slate-500 group-hover:text-indigo-600 transition-colors">
                        <IconBox />
                      </div>
          ...
          <div className="space-y-6">
            ...
            {!loading && courierStats.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  {courierStats.map((c) => (
                    <a key={c.id} href={`/dashboard/usuarios/${c.id}`} className="block bg-white border border-slate-200 rounded-md px-4 py-3 hover:border-indigo-200 hover:bg-indigo-50/30 transition-colors">
                      <div className="flex items-center justify-between mb-1">
                        <p className="font-semibold text-slate-900 text-sm truncate">{c.name}</p>
                        <span className={`font-mono text-xs font-bold px-2 py-0.5 ${c.todayCount > 0 ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-500"}`}>
                          {c.todayCount} {lang === "en" ? "today" : "hoy"}
                        </span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="flex-1 h-1.5 bg-slate-100 overflow-hidden">
                          <div className="h-full bg-emerald-500" style={{ width: `${c.successRate}%` }} />
                        </div>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {todayPickups.slice(0, 5).map((p) => (
                    <Link key={p.id} href={`/dashboard/solicitudes/${p.id}`}>
                      <div className="bg-white border border-slate-200 rounded-md px-3 py-2.5 flex items-center gap-2.5 hover:border-indigo-200 hover:bg-indigo-50/30 transition-colors">
                        <StatusBadge status={p.status} />
